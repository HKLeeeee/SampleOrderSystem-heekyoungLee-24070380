...
**시간 시뮬레이션 방식 (A)** 채택: 메뉴 진입 시점의 시스템 시각을 기준으로 경과 시간을 계산하여 완료 여부를 자동 판정한다. 별도 수동 명령 불필요.
...
```
Phase 1. 프로젝트 골격 + Harness  ← 현재
Phase 2. 도메인 모델 + 단위 테스트
Phase 3. 영속성 계층 (JSON Repository)
Phase 4. 기능 구현 (4-1 시료 → 4-2 주문 → 4-3 승인 → 4-4 생산 → 4-5 모니터링 → 4-6 출고)
Phase 5. 메인 메뉴 통합 + 시나리오 검증
Phase 6. 리팩터링 + 문서 정리
```
